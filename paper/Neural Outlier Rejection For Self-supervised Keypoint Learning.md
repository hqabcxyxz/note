...
在处理时,空间变换包括先将图片裁剪到原始的 0.7 大小.然后缩放范围是$[0.8,1.2]$,旋转范围是$[0,\frac{\pi}{4}]$,perspective范围是[0,0.2].光学色彩变换包括,幅值为0.02的高斯噪声,使用核大小为[1,3,5]大高斯模糊,亮度调整[0.5,1.5],对比度[0.5,1.5],饱和度[0.8,1.2],色调[-0.2,0.2].并且0.5机率混洗颜色通道和将图片转换成灰度图.
